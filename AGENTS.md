...
Name/port discipline (textual calculi).
    • Fernández–Mackie / lightweight: names are wires; each name occurs ≤2 (rule-bound names occur exactly twice).
    • HVM4: vars are affine; reuse only via dup/sup or auto-dup; scope is non-lexical (scopeless).
    • If a name occurs 3+ times, a DUP is missing; if it occurs once where internal wiring is expected, an ERA is missing. Fix wiring before reasoning.
...
HVM4 core interactions (compact)
Rule | LHS pattern | Effect (sketch)
APP-LAM | (λx.f a) | x ← a; f
APP-SUP | (&L{f,g} a) | !A &L= a; &L{(f A₀),(g A₁)}
DUP-SUP | !X &L= &R{a,b} | if L==R: X₀←a; X₁←b; else: !A &L= a; !B &L= b; X₀←&R{A₀,B₀}; X₁←&R{A₁,B₁}
DUP-LAM | !F &L= λx.f | F₀←λ$x0.G₀; F₁←λ$x1.G₁; x←&L{$x0,$x1}; !G &L= f
